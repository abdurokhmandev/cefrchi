...
if HAS_GOOGLE:
    # Read API key from pydantic Settings
    ...
    if not api_key:
        # If API key not provided, disable GenAI usage to avoid runtime errors
        print("Warning: GOOGLE_API_KEY not provided — GenAI disabled.")
        HAS_GOOGLE = False
    else:
    ...
    # Fallback stubs when google-genai is not available; prevents the whole bot from crashing
    async def transcribe(audio_path: str) -> str:
        return ""

    async def get_feedback(transcript: str, topic: str, lang: str, part: int = 1) -> dict:
        return {"error": "google-genai not installed or GOOGLE_API_KEY missing", "band": "—", "cefr": "—", "full_text": "Xizmat mavjud emas", "grammar": "", "vocab": ""}
